        ...
        if datesArr:
            ...
            for date in datesArr:
                ...
                if alterDate is None:
                    newTag = ET.Element('date', attrib={'date eId': Id, 'date': numFormat})
                else:
                    newTag = ET.Element('date', attrib={'date eId': Id, 'alternativeTo': alterDate, 'date': numFormat})
        ...
        if datesArr:
            ...
            for date in datesArr:
                ...
                if len(re.findall(HebrewDates, a)) > 0:
                    ...
                    alterDate = hebToGreg.get(numFormat, None)
                if alterDate is None:
                    newTag = ET.Element('date', attrib={'eId': Id, 'date': numFormat})
                else:
                    newTag = ET.Element('date', attrib={'eId': Id, 'alternativeTo': alterDate, 'date': numFormat})
                newTag.text = " ".join(date)
                txt = txt[txt.index(" ".join(date)) + len(" ".join(date)):]
                if i < len(datesArr)-1:
                    next = removeBlankCells(datesArr[i+1])
                    next = " ".join(next)
                    newTag.tail = txt[0: txt.index(next)]
                else:
                    newTag.tail = txt
                node.append(newTag)
                i = i+1
